...
- **Last checkpoint:** 33 — **resource bounds and replication readiness**
- **Next task:** load balancer lab (nginx + 3 replicas against one Postgres), then
  `FRONTEND_BLUEPRINT.md` V1.0, then `CONTEXT_HANDOFF.md`
...
| Replication readiness               | ✅ Complete — pool bounds, server timeouts, instance identity |
...
### ✅ Checkpoint 27 — **save implementation details** · `PUT /api/changecontrols/{ccID}/implementation` (endpoint **23** — a new one)

**A deliberate addition to the API plan, taking the total to 23.** CC Owner only,
`In Implementation` only, fields 29–33.

**Why it exists (decision #43).** The plan had **no save endpoint for `In Implementation`**,
which left T6 as the only way those five fields could ever be written — so T6 would **save and
validate and sign and transition** in one request. Two problems: a validation failure rolls
back everything, so a user who mistypes one field **loses the other four and retypes them**;
and it makes T6 the only transition doing four jobs. `Initiated` already solved this with
save-then-submit; the same reasoning applies here.

**This reverses the checkpoint 20 position** (flag #18), which argued implementation details
are a "completion report written in one sitting" rather than a draft. That still has some
force — but it does not answer the retype problem.

**The payoff: T6 becomes a copy of T2** — body is `{email, password}`, validation runs against
stored values, no field writes.

**Structurally identical to Save Draft, five blocks instead of 24** — the same
`map[string]json.RawMessage` decode, the same pre-seeded params, the same whitelist-and-collect
rejection, the same `changed` bool and re-fetch-before-commit.

**Two things specific to this state:**

- **`post_implementation_issues` is a DROPDOWN, not a textarea** — the field reference flags
  this explicitly, and the three values include an **ASCII hyphen** in
  `Issues requiring follow-up` (DB §6.5)
- **No date rule on `actual_implementation_date`** (decision #44). It is retrospective, so a
  future date is wrong — but enforcing that _here_ would block a user saving on Monday for work
  scheduled Wednesday. **Save takes any date; T6 rejects a future one.** Same split as T2's
  business-day rules

**No audit rows** — none of fields 29–33 appear in §6.6.2's auditable list.

| Check                                                                                            | Verified |
| ------------------------------------------------------------------------------------------------ | -------- |
| Partial save leaves unmentioned fields untouched — the absent-key mechanism on a second endpoint | ✅       |
| `null` and whitespace-only both clear; a repeat save is a **no-op**                              | ✅       |
| **En-dash `Issues requiring follow–up` → 400**; ASCII hyphen → 200                               | ✅       |
| Date without a time → 400 · `""` → 400 (only `null` clears) · **future date → 200**              | ✅       |
| Unknown keys collected and sorted, **nothing written alongside them**                            | ✅       |
| **`PUT /{ccID}` returns 409 on this record** — the two save endpoints guard their own states     | ✅       |
...
### ✅ Checkpoint 32 — **API documentation, CORS, and guardrail alignment** (no new endpoints)

Post-build work. Nothing here changes a handler's behaviour.

#### Guardrail documents brought to V1.2

All four now describe **what was built**, not what was intended. Until this, the
specifications contradicted the code in eleven places, and a future session reading them
would have "corrected" deliberate decisions back.

| Doc                              |                                                                                                                                                                                                                                                            |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **BRD V1.2**                     | Five new Phase 1 limitations (L11–L15); BR-8.2.13 narrowed to PDF only; BR-8.4.11 made all-or-nothing; SC-6's contradicting clause removed; §9.5.2 gains the Recent Activity section; US-AD-03 gains the deactivation block; new BR-8.2.16 password policy |
| **DB Design V1.2**               | Column count 48 → **50** (the parenthetical was always right; the headline was wrong); `idx_cc_last_updated_on` added to the index list; count 15 → 16                                                                                                     |
| **`CC_Field_Reference.md` V1.2** | 🔒 added to fields 13 and 14; field 24 marked not implemented; field 29's future-date rule; field 34 PDF-only; **a new Audit Scope section listing the nine tracked fields**                                                                               |
| **Security Matrix V2.1**         | Updated separately                                                                                                                                                                                                                                         |
| **HTML prototypes**              | En-dash option values fixed; "Username" → "Email" in the signature modal; Supporting Documents block removed; profile screens made read-only; blocked-role banner corrected                                                                                |

**`GO_CODING_GUIDE.md`** written — 18 sections, 114 rules, 109 code examples — and
**supersedes `BACKEND_BLUEPRINT.md`**, which was written before the build and described
intentions.

#### `docs/` — OpenAPI 3.0.3 + Swagger UI

**23 operations, 46 schemas, hand-written from the handler code.** Deliberately _not_
generated from the Postman collection: Postman records the responses that happened to be
saved, with no nullability, no enums and no required/optional distinction. The spec records
what the API **accepts and guarantees**.

Served from the binary via `go:embed` at **`GET /docs`** and **`GET /docs/openapi.yaml`** —
two ordinary handlers, **not middleware**, public because Swagger UI cannot send a bearer
token to fetch its own specification. Outside `/api`, since documentation is not API surface.

The spec is **hand-written, so it must be updated in the same commit as any endpoint
change.** A stale spec is worse than none, because it is trusted.

#### CORS (decision #48)

`middlewareCORS`, wrapping **the whole mux** rather than individual routes — a preflight
`OPTIONS` on a path with no registered handler must still be answered, and per-route
middleware never runs when no route matches. Origins come from `ALLOWED_ORIGINS` in `.env`.

**Required for the frontend**, not optional: the Svelte dev server on `:5173` is a different
origin from the API on `:1304`, so without this every request would be blocked.

| Check                                                | Verified |
| ---------------------------------------------------- | -------- |
| `POST /login` from Swagger UI → **200 with a token** | ✅       |
| `GET /dashboard` with the token → all four blocks    | ✅       |
| Postman unaffected — it sends no `Origin` header     | ✅       |
| `/docs` served from the binary, no Python server     | ✅       |

**Three things about CORS worth remembering** (the rest is orchestration):

1. It is a **browser** rule, not an API security control. Postman and curl ignore it entirely,
   which is why the API worked for months without it.
2. **A blocked request still runs.** The browser sends it, the handler executes, the database
   is written — and only then does the browser refuse to hand the response to JavaScript.
3. **Never `*`** with credentials. Echo back only an origin from an explicit list, and add
   `Vary: Origin`.

---

### ✅ Checkpoint 33 — **resource bounds and replication readiness** (no new endpoints)

Preparation for running the API as multiple replicas behind a load balancer. Nothing here
changes what a handler does; all of it changes what the **process** does under contention,
and only one of the six is meaningless at a single instance.

**The organising idea.** Most of what makes a service ready to scale horizontally is
ordinary single-instance engineering that had been skipped: bounded resources, timeouts,
identity, structured logs. Horizontal scaling does not demand a new category of work — it
demands the work that was always correct, and punishes its absence loudly. The genuinely
new problems are only three: **which instance served this**, **coordination on a resource
that is shared but limited**, and **state that cannot live in a process**. The third does
not arise here, because the API was already stateless — configuration comes from the
environment, sessions are JWTs, and nothing is held in memory between requests.

#### The six changes

| #   | Change                                                             | New at replication?                                  |
| --- | ------------------------------------------------------------------ | ---------------------------------------------------- |
| 1   | Pool bounded; ceiling read from `DB_MAX_OPEN_CONNS`                | The coordination problem is new                      |
| 2   | `ReadHeaderTimeout` / `WriteTimeout` / `IdleTimeout` on the server | No — always correct                                  |
| 3   | `INSTANCE_ID` on every log line and as `X-Instance-ID`             | **Yes — a constant at one instance**                 |
| 4   | `middlewareLogging` moved outside the mux                          | No — always correct                                  |
| 5   | `responseRecorder` captures the status for the finish line         | No — always correct                                  |
| 6   | 30-second request-scoped context deadline                          | No — but the leak it prevents costs more per replica |

Recorded as decisions #49–#54; costs recorded as flags #26–#30.

#### Two earlier entries corrected

Checkpoint 33 made two existing records false, and both were amended rather than left to
mislead a future session:

- **Decision #48** claimed CORS wraps the mux "unlike every other middleware in this
  codebase." `middlewareLogging` now does the same, for the same reason.
- **Flag #25** stated that preflight `OPTIONS` requests never reach `middlewareLogging`.
  They now do.
...
| Check                                                                                     | Verified |
| ----------------------------------------------------------------------------------------- | -------- |
| Startup logs `max_open_conns=25` from `.env`, not the built-in default of 10              | ✅       |
| `X-Instance-ID` present on 200, 404, 401 and the 204 preflight                            | ✅       |
| An unmatched path produces **both** log lines, with `status=404`                          | ✅       |
| A preflight `OPTIONS` produces both log lines and returns 204                             | ✅       |
| Request IDs stable within a request, unique between requests                              | ✅       |
| Status recorded through both write paths — `http.Error` (404) and `respondWithJSON` (401) | ✅       |
| `status=201` on create — non-200 success codes recorded correctly                         | ✅       |
| Full workflow unaffected: create → save draft → T2 → T4 approve → upload → download       | ✅       |
| Slowest observed request 324 ms (login, argon2id) — two orders below the 30 s deadline    | ✅       |
...
### 5. Two-line error classification (flags #26, #27)

`context.DeadlineExceeded` → **504**, and `context.Canceled` → logged at INFO rather than
ERROR, since it means the client disconnected and not that anything failed. The same edit at
the same call sites, so they are one task rather than two — realistically 60–100 insertions
across 23 handlers. **Not urgent:** the deadline already releases the pool connection, which
was the structural problem; what remains is accuracy of the status code and of any error rate
derived from the logs. Best done alongside the helper extraction in item 1, since both touch
every handler.
...
| 48  | **CORS middleware wraps the mux; API documentation is embedded in the binary**                                                                                                                                                                                                                             | _CORS:_ it wraps the **whole mux**, unlike every other middleware in this codebase, because a preflight `OPTIONS` arrives on a path with no registered handler — per-route middleware would never run. Origins are read from `ALLOWED_ORIGINS` in `.env` rather than hardcoded, so dev and production differ without a code change. **Never `*`**: a wildcard cannot be combined with credentials and would let any site call the API from a logged-in user's browser. _Docs:_ `go:embed` compiles `openapi.yaml` and `index.html` into the binary, so they deploy with the code they describe and cannot go missing. Editing the spec requires a rebuild — correct, since the spec should change in the same commit as the endpoint. Both routes are **public and outside `/api`**: Swagger UI cannot authenticate to fetch its own specification, and documentation is not API surface **Amended at checkpoint 33 (decision #52):** "unlike every other middleware in this codebase" no longer holds — `middlewareLogging` now also wraps the mux, and wraps `middlewareCORS` in turn, for exactly the reason given here: per-route middleware never runs when no route matches.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 49  | **The connection pool is bounded, and its ceiling is read from `DB_MAX_OPEN_CONNS` rather than hardcoded**                                                                                                                                                                                                 | `sql.Open` defaults `MaxOpenConns` to **unlimited** and `MaxIdleConns` to **2** — survivable for one process, wrong for several. Each replica holds an independent pool with no knowledge of its peers, while Postgres enforces one global `max_connections` (default 100, ~97 usable after superuser reservations). **The limit is per-process; the resource it consumes is shared.** Three replicas at 25 is 75; a fourth takes it to 100 and nothing warns you, because `api-4` cannot see that `api-1` exists. Left unbounded, contention surfaces as `FATAL: sorry, too many clients already` — a hard error returned to the caller. Bounded, the same contention becomes queueing inside the pool: request N+1 waits rather than failing. **The principle: bounded resources degrade, unbounded resources collapse** — a slow request is recoverable where a failed one usually is not, and the queue now sits inside the process where it can be observed. Settings are applied **between `Open` and `Ping`**, so the verification connection is created under the configured limits rather than the defaults. `MaxIdleConns` matches the ceiling to avoid reconnect churn; `ConnMaxIdleTime` (5 min) reaps idle connections when traffic drops; `ConnMaxLifetime` (1 h) recycles even busy connections, which matters when a proxy or firewall silently drops long-lived TCP sessions. Environment-driven so replica count can change without a rebuild, and so the collapse-versus-degrade behaviour can be demonstrated by varying one variable. **PgBouncer is the structural answer at scale** (flag #30)                                                      |
| 50  | **Three server timeouts set; `ReadTimeout` deliberately left unset**                                                                                                                                                                                                                                       | `http.Server` was constructed with only `Addr` and `Handler`, so every timeout defaulted to zero — no limit. A client could open a connection and never finish sending, and it would be held indefinitely: the Slowloris shape, where a few clients dribbling bytes occupy every connection the server will accept. **`ReadTimeout` is the one that cannot be used here, and the reason generalises: a constraint must be expressed at a layer that knows enough to vary it.** These are fields on `http.Server`, and the clock starts when the connection is accepted — _before_ the mux has read the request line and matched a route — so there is no way to say "15 seconds for `/api/me`, 120 for the upload." Since `ReadTimeout` covers the request **body**, any value tight enough to defend against a slow client would also kill a legitimate 10 MB evidence upload on a slow link. **`ReadHeaderTimeout` (10 s)** is used instead: headers are small and arrive immediately in any real request regardless of route, so one value is correct everywhere, and it is the bound that actually defends against slow headers. Body size is constrained per-route by `http.MaxBytesReader` in `HandlerUploadFile` — the handler knows which route it is on; the server does not. **`WriteTimeout` (30 s)** is roughly twenty times what a 10 MB transfer needs while still bounding a stalled reader. **`IdleTimeout` (60 s)** caps keep-alive reuse; note for when a reverse proxy is put in front that the proxy's keepalive must be **lower** than this, or it will occasionally dispatch onto a connection the server is closing, surfacing as intermittent 502s |
| 51  | **Each process resolves an `INSTANCE_ID`, attaches it to every log line, and returns it as `X-Instance-ID`**                                                                                                                                                                                               | **The only one of the six that is meaningless at a single instance** — one process makes it a constant on every line. It exists because horizontal scaling makes "which process handled this?" a question that cannot otherwise be answered: three replicas writing to one aggregated stream are indistinguishable without it, and "instance 2 is throwing 500s while the other two are fine" is invisible. Resolution order is `INSTANCE_ID` → hostname → `"unknown"`. The hostname is the machine name on bare metal and the **container ID** under Docker, so it is a fallback rather than the intended source — `docker run --name` does **not** set a container's hostname, and each replica is given the variable explicitly. Deliberately **not** added to `.env`: bare metal wants the hostname fallback, and the value is per-container. The header is set as the **first statement** in the logging middleware, because once `WriteHeader` has been called the header map is frozen and a later `Set` is a **silent** no-op — no error, no panic, the header is simply absent. `X-Instance-ID` is added to `Access-Control-Expose-Headers` on both the preflight and the actual response, since a browser hides every non-safelisted header from JavaScript                                                                                                                                                                                                                                                                                                                                                                                                      |
| 52  | **`middlewareLogging` moves off the individual routes and wraps the mux, _outside_ `middlewareCORS` — amends decision #48**                                                                                                                                                                                | #48 recorded that CORS wraps the whole mux "unlike every other middleware in this codebase." **That is no longer true**, and for the same reason it gave: per-route middleware never runs when no route matches. Applied per route, logging missed everything the mux did not match — **an unmatched path produced no log line at all** — and missed every preflight `OPTIONS`, which CORS answers and returns before the mux is reached. Both classes of request were invisible. **The nesting order is load-bearing, and it is logging-outermost rather than CORS-outermost.** Reversed, `middlewareCORS` would call `logging.LoggerFrom` for its rejected-origin warning before any request-scoped logger existed in the context, and preflights would still escape before being logged. **The invariant: logging is the outermost layer, because anything outside it is invisible.** Closes the last sentence of flag #25                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 53  | **`responseRecorder` wraps the `ResponseWriter` so the finish line can record the status**                                                                                                                                                                                                                 | The finish line reported method, path and duration — so a 404, a 401 and a 200 were **indistinguishable**. Once logging wrapped the mux (#52), unmatched paths produced a line that said nothing about what happened, and error rate per endpoint could not be derived from the log at all. `http.ResponseWriter` is **write-only**: `WriteHeader` serialises the code onto the connection and `net/http` retains nothing, so middleware sitting outside the handler has nothing to query once `ServeHTTP` returns. Capturing it requires intercepting the call — **this is the standard Go idiom, not a workaround.** chi, gorilla, Echo and Gin all do exactly this and merely hide the wrapper behind a `Status()` accessor; the plumbing is visible here because there is no framework to hide it. The struct **embeds** the interface, so `Header` and `Write` are promoted and forwarded untouched; only `WriteHeader` is overridden, and it stores the code **before** calling through, so the bytes on the wire are unchanged. Initialised to **200**, because a handler that calls `Write` without `WriteHeader` causes `net/http` to send 200 implicitly and the log should record what the client received. Cost recorded as flag #28                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 54  | **Every request carries a 30-second context deadline, applied uniformly rather than exempting the file routes**                                                                                                                                                                                            | The server timeouts (#50) bound the **connection** from outside; neither stops the handler. A query that hangs leaves its goroutine running and its pool connection **checked out for the life of the process** — the client eventually gives up, but the connection never returns. **This is what makes the bound in #49 real:** bounding a pool converts contention into queueing only if connections come back to be queued for, and a leaked connection is not queued for, it is simply gone. At `MaxOpenConns=5` per replica, one stuck query has permanently removed 20 % of that instance's database capacity. Because every sqlc call and `BeginTx` takes `r.Context()`, the cancellation reaches the driver and the connection is released. **30 s matches `WriteTimeout`:** a longer deadline could never fire, since the connection is already gone by then, and a shorter one would cut off a legitimate upload — the deadline starts when the request **arrives**, so it runs during the body transfer, not only during database work. One value for every route rather than exempting `/files/*`, accepted knowingly: a looser bound than a JSON endpoint needs, in exchange for middleware with no route knowledge in it. `defer cancel()` is not optional — without it the timer and its goroutine survive every request until the deadline expires. Error mapping deferred (flags #26, #27)                                                                                                                                                                                                                                                               |
...
| F4  | ~~Disable the active/inactive toggle for the current user's own row.~~ **ALREADY CORRECT** — verified in `settings-admin.html`: the `(you)` row's toggle carries `disabled`. The original note came from reading a screenshot rather than the markup. No change needed                                                                                                                                                                                                                                                                                                                                                                                  | checkpoint 24; corrected on review                 |
...
| F18 | **`In Implementation` is save-then-submit too — the same two-step flow as `Initiated`.** The screen needs its own **Save** button calling `PUT /{ccID}/implementation`; **Submit for Final Approval** must be disabled while the form is dirty, because T6 carries no field values and validates only what is stored. Also: the **date picker for Actual Implementation Date should disable future dates** — the field is retrospective and T6 rejects a future value, so the UI should prevent it rather than let the user discover it at submission                                                                                                   | checkpoint 27, decisions #43/#44                   |
...
| 18  | ~~**No partial saving during `In Implementation`.**~~ **CLOSED at checkpoint 27 — the opposite decision was taken.** A 23rd endpoint (`PUT /{ccID}/implementation`) was added rather than having T6 carry the five fields, because a validation failure in a combined save-and-submit would make the user retype everything (decision #43)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **Closed**                                                                                                         |
...
| 25  | **A CORS-blocked request is invisible in the logs.** The browser sends it, the handler executes and logs a normal 200, and only then does the browser discard the response — so a frontend origin missing from `ALLOWED_ORIGINS` looks like a working request server-side while failing entirely client-side. Inherent: the server cannot see the browser's decision. **Mitigated** by logging a `cors origin rejected` warning when an `Origin` header is present but not allowed, which turns the confusing case into a searchable line. ~~Preflight `OPTIONS` requests never reach `middlewareLogging` at all, since CORS answers and returns first.~~ **Corrected at checkpoint 33 (decision #52):** they now do — `middlewareLogging` wraps `middlewareCORS` rather than sitting inside it, so a preflight logs both lines and carries `X-Instance-ID`. The rest of this flag stands: the browser's decision remains invisible to the server, and the `cors origin rejected` warning is still the only server-side trace | Mitigated; inherent limitation noted                                                                               |
| 26  | **A timed-out request returns 500, not 504.** A cancelled context surfaces in handlers as a plain error and falls into the existing `respondWithError(w, "Something went wrong", 500)` branch, so a request that exceeded its deadline is indistinguishable in the response from a database failure. Correcting it means an `errors.Is(err, context.DeadlineExceeded)` check at **every database call site across 23 handlers** — `HandlerUploadFile` alone has six, `HandlerSaveDraft` is ~700 lines — realistically 60–100 insertions for a more accurate status code. **Deliberately deferred: the deadline itself is a small fraction of the work and nearly all of the value; the mapping is the reverse.** Timed-out requests remain identifiable in the log by a `duration_ms` sitting at the deadline                                                                                                                                                                                                                 | Deferred — decision #54                                                                                            |
| 27  | **`context.Canceled` is logged at ERROR alongside real faults.** It means the **client disconnected**, not that anything failed — nobody is waiting for the response, so the write to a dead socket is harmless. The cost is not the wasted write but the **classification**: any error rate derived from these logs counts closed browser tabs as server errors. Same edit at the same call sites as flag #26, so the two are one task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Deferred — decision #54                                                                                            |
| 28  | **`responseRecorder` hides the other interfaces the underlying writer implements.** `net/http`'s `ResponseWriter` also satisfies `http.Flusher` and `http.Hijacker`; the wrapper does not, so a type assertion such as `w.(http.Flusher)` inside a handler now **fails where it previously succeeded**. Nothing in the codebase does this today — there is no streaming, SSE or WebSocket endpoint — so nothing is broken. **It becomes live the moment one is added**, since hijacking the connection is how a WebSocket upgrade works. The fix is a three-line passthrough per interface                                                                                                                                                                                                                                                                                                                                                                                                                                    | Noted — decision #53                                                                                               |
| 29  | **File logging is the wrong destination inside a container.** `NewLogger("logs")` writes JSON to `logs/app.log` in the container's writable layer. Replicated, that is **N separate files, each inside its own container, each destroyed with it** — and `docker logs` shows only stdout, so the text handler's output is visible while the structured JSON is not. The twelve-factor position is that a process should not manage log files at all: write the event stream to stdout and let the runtime collect it. `INSTANCE_ID` is what makes that work, since replicas then share one stream that can be filtered per instance. **Reframes flag #10** — rotation is the bare-metal answer; in a container the file should not exist                                                                                                                                                                                                                                                                                      | Open — decide before any containerised deployment                                                                  |
| 30  | **The pool budget is maintained by hand and nothing enforces it.** `replicas × DB_MAX_OPEN_CONNS + headroom ≤ max_connections` is an invariant held in deploy configuration, not in code. Adding a replica six months from now silently exceeds it, and the symptom appears at the **database**, not at the service that caused it. **PgBouncer is the structural answer** — one component that can see the total, turning "every replica must be configured correctly forever" into a single owner of the budget. Deferred to the database-scaling work                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Noted — decision #49                                                                                               |
...
**`.env` — two additions from checkpoint 33, and one rule about quoting.**

| Variable            |                                                                                                                                                                                                                                                 |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DB_MAX_OPEN_CONNS` | Connection-pool ceiling **per process**; defaults to 10 when unset. `replicas × this` must stay under Postgres's `max_connections` (flag #30). Set to 25 for bare metal                                                                         |
| `INSTANCE_ID`       | **Not in `.env`, deliberately.** Identifies the replica in logs and `X-Instance-ID`; falls back to the hostname, which is what bare metal wants. Set per container with `-e`, since `docker run --name` does **not** set a container's hostname |

**Values must not be quoted.** `godotenv` strips surrounding quotes; `docker run
--env-file` and Compose's `env_file` do **not**. A quoted `JWT_SECRET` carries literal
quote characters into the container, so a token signed on bare metal would fail to
validate there — and a quoted `DB_URL` fails to parse. Unquoted is the only form correct
