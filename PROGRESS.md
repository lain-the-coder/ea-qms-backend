...
- **Last checkpoint:** 20 — `PUT /api/changecontrols/{ccID}` · **13 of 22 · Group 3 COMPLETE**
- **Next task:** checkpoint 21 — **T2 submit** (endpoint 15) — the hardest endpoint in the project
...
| API — Group 5 Workflow (15–19)      | ⬜ **Next** — T2 first, written fully inline      |
...
### ⬜ Checkpoint 21 — **T2 submit** (endpoint 15) — the hardest endpoint in the project

`POST /api/changecontrols/{ccID}/submit` — `Initiated` → `Pending Implementation Approval`.
Owner-only, `Initiated`-only.

**Save Draft was _long_; T2 is _hard_.** Save Draft repeated one mechanism 24 times. T2 is
seven new mechanisms at once:

| Piece                                | Note                                                                                                                                                                                                 |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **20 mandatory fields**              | **collect-all** into one 400 (BR-8.2.6) — never fail-on-first                                                                                                                                        |
| **Business-day date maths**          | `proposed_implementation_date` ≥ **2** weekdays out, `target_closure_date` ≥ **10**, both re-validated now (a stale draft date must be corrected). Weekdays only — **no public holidays in Phase 1** |
| **E-signature verification**         | username + password in the body; **must match the logged-in user** (BR-8.8.3); same argon2id verify. **Never log or store the raw password**                                                         |
| **State change**                     | plus `implementation_approval_status` → `Pending`                                                                                                                                                    |
| **First `esignatures` row**          | transition `T2`, meaning `Submitted for Implementation Approval`                                                                                                                                     |
| **Multiple audit rows**              | `StateChanged` + `SignatureCaptured`, sharing one timestamp                                                                                                                                          |
| **A failure path that still writes** | a failed signature writes `SignatureFailed` via `cfg.db` **outside any transaction**, returns 401, record untouched                                                                                  |

That last one is the subtle piece: an audit row that must **survive** when everything else
rolls back.

**§0: write it fully inline.** No `verifySignature`, no `writeAudit` — those get extracted
after T3 and T4/5 exist, with three real examples in front of you rather than one guess.

**Also decide at T2:** whether `implementation_window_end` must be after `_start` (flag #20 —
no guardrail doc states the rule).

**After T2 the curve descends** — T3 and T4/5 reuse its shape, then the helpers get extracted;
T6, T7/8 are variations; files and signatures are comparatively small.
